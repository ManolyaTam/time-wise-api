...
const projectController = {
  createProject: async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      const existingProject = await User.findOne({ email: userEmail, 'projects.name': name });
  ...
  getAllProjectsData: async (req: Request, res: Response, next: NextFunction) => {
    try {
      const userEmail = req.user?.email;
  ...
  getAllProjectsNames: async (req: Request, res: Response, next: NextFunction) => {
    try {
      const userEmail = req.user?.email;
